...
299. **[NEW] Add exit-intent popup with 5% discount code on ALL landing pages** — Detect mouse leaving viewport (mouseleave on document). Show modal: "Czekaj! Odbierz 5% zniżki na pierwsze zamówienie" with email input. Captures leads from users who were about to leave without buying. Store email + discount code (FIRST5) via Formspree. Target: porownanie.html, ranking-nootropikow.html, sesja.html, matura.html, powrot-do-szkoly.html. Industry avg: 3-5% exit conversion. Estimated: 1.5 hours.

300. **[NEW] Create "Suplementy a medytacja — czy nootropiki wspierają mindfulness?" blog post** — Target "suplementy medytacja", "nootropiki mindfulness". Growing Polish wellness market (~800 searches/month). Cover: neurochemistry of focus vs present-moment awareness, cytykolina for attentional control, ALA for oxidative stress from meditation-induced deep breathing, β-CD for delivery. Position CogniCit as complement to meditation practice, not replacement. Estimated: 2 hours.

301. **[NEW] Add "Ile osób kupiło" social proof ticker to index.html hero** — Simulated live counter showing "X osób kupiło CogniCit w tym miesiącu" in hero section below newsletter counter. Base count 89, increments daily (0-3/day). Creates urgency + social proof at top of funnel. Similar to existing "osób zapisało się na premierę" counter. Estimated: 30 minutes.

302. **[NEW] Create "Video testimonials" section with embedded customer story videos** — Convert 3-5 text testimonials from opinie.html into short video testimonials. Even phone-recorded clips (30-60s) with customer sharing their CogniCit experience dramatically increase trust vs text-only. Host on YouTube (unlisted), embed on index.html + produkt.html. Estimated: 2 hours (content creation + embed).

303. **[NEW] Add "FAQ accordion to ALL landing pages" (matura, sesja, powrot-do-szkoly, ranking)** — Currently FAQ sections exist on only some pages. Add the same expand/collapse FAQ accordion pattern to all seasonal landing pages. 3-4 questions per page relevant to that landing page's audience (students, parents, etc.). FAQPage JSON-LD schema on each page for Google rich snippets. Estimated: 1 hour per page, batch all at once.

304. **[NEW] Implement critical CSS inlining for LCP optimization** — Extract above-the-fold CSS from index.html and inline in <style> block in <head>. Defer remaining CSS. Reduces render-blocking resources and improves LCP by 0.3-0.8s on mobile. Use existing inline-style sections as reference for what is "critical". Estimated: 1.5 hours.

305. **[NEW] Add "Polecane produkty" (related products) carousel to blog post pages** — After each blog post, show 3 related content cards or product recommendation. Cross-promotes CogniCit product within educational content. Uses existing blog card styling. Target: all 37 blog posts. Estimated: 30 minutes.

306. **[NEW] Create "Suplementy a dieta ketogeniczna — czy można łączyć?" blog post** — Target "suplementy dieta ketogeniczna", "keto nootropiki". Growing keto market in Poland. Cover: ALA as fat-soluble antioxidant fits keto, cytykolina no interaction, β-CD delivery mechanism keto-friendly, caffeine-free = safe for keto fast. Estimated: 2 hours.

307. **[NEW] Add scroll-triggered progress indicator to all long-form content pages** — Visual reading progress bar at top of page (like on YouTube). Shows visitors how much content remains. Particularly useful on long-form pages: porownanie.html (27KB), faq-produkt.html (28KB), sesja.html (27KB), ranking-nootropikow.html (45KB). Uses simple JS with requestAnimationFrame. Estimated: 45 minutes.
